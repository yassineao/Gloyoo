...
const styles = {
  bodyText: "text-sm leading-7 text-brand-graySoft sm:text-base sm:leading-8 break-words",
  ...
  tableText: "min-w-full text-left text-xs text-brand-graySoft sm:text-sm",
      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-sm leading-6 text-brand-graySoft sm:leading-7 sm:text-base break-words">
              {right.description}
            </p>
          </div>
        </div>
  ...
  <div className="mt-5 grid gap-3 sm:mt-6 sm:gap-4 md:grid-cols-2">
    {cards.map((card, index) => (
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-brand-graySoft sm:mt-3 sm:text-base sm:leading-7 break-words">
          {card.value}
        </p>
      </article>
    ))}
  ...
  <div className="mt-4 grid gap-3 md:mt-6 md:gap-4 md:grid-cols-2">
    {categories.map((category, index) => (
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-brand-graySoft sm:mt-3 sm:text-base sm:leading-7">
      ...
      <div className="mx-auto max-w-4xl">
        <article className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-md sm:p-8 lg:p-10">
          ...
          <header>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-6 text-brand-graySoft sm:mt-6 sm:text-base sm:leading-7 lg:text-lg lg:leading-8 break-words">
              {intro}
            </p>
          </header>
            ...
            <div className="mt-5 flex flex-wrap gap-2 sm:mt-6 sm:gap-3" role="list">
              {heroMeta.map((item, index) => (
                <span
                  key={index}
                  role="listitem"
                  className="rounded-full border border-white/10 bg-black/30 px-3 py-1.5 text-xs text-brand-graySoft sm:px-4 sm:py-2 sm:text-sm break-words"
                >
                  {item}
                </span>
              ))}
          ...
          {contents && contents.length > 0 && (
            <nav className="mt-6 rounded-2xl border border-white/8 bg-black/20 p-4 sm:mt-8 sm:p-6" aria-label="Page contents">
              <ul className="mt-3 grid gap-2.5 text-xs leading-6 text-brand-graySoft sm:mt-4 sm:grid-cols-2 sm:gap-3 sm:text-sm sm:leading-7">
